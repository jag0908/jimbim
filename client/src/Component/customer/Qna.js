...
function Qna() {
    ...
    const customStyles = {
        ...
        content: { left: 0, margin: "auto", width: "500px", height: "650px", padding: 0, overflow: "hidden" },
    };
    ...
    useEffect(() => {
        if (loginUser.userid) {
            ...
            .then(result => setMember(result.data.member));
        }
    }, []);
    ...
    async function onSubmit() {
        ...
        setIsOpen(false); setTitle(''); setContent('');

        // refresh list
        const result = await jaxios.get(`/api/customer/getQnaList`, { params: { userid: loginUser.userid, page: 1 } });
        setQnaList(result.data.qnaList);
        setPaging(result.data.paging);
        const arr = [];
        for (let i = result.data.paging.beginPage; i <= result.data.paging.endPage; i++) arr.push(i);
        ...
        <div className="customercontainer customer-qna">
            ...
            <div className="customer">
                ...
                <div className="qnaTable">
                    ...
                    {qnaList.length !== 0 ? qnaList.map((qna, idx) => (
                        <div key={idx} className="qnaList" onClick={() => navigate(`/customer/qna/${qna.qnaId}`)}>
                            ...
                            <div className="qnaField">{qna.indate?.substring(0,10)}</div>
                        </div>
                    )) : (loginUser.userid ? <div>아직 문의가 없습니다</div> : <div>로그인 후 사용 가능합니다</div>)}
                    {/* 페이징 */}
                    {qnaList.length !== 0 && (
                        <div id="paging" style={{textAlign: "center", padding: "10px"}}>
                            {paging.prev && <span style={{cursor:"pointer"}} onClick={() => onPageMove(paging.beginPage-1)}> ◀ </span>}
                            {beginEnd.map((page, idx) => (
                                <span key={idx} style={page === paging.page ? {fontWeight:'bold', cursor:"pointer"} : {cursor:"pointer"}} onClick={() => onPageMove(page)}>
                                    &nbsp;{page}&nbsp;
                                </span>
                            ))}
